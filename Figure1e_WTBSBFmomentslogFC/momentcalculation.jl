...
function bootstrap_mean_std(genedata, gene; subsampling = true, cellssubsampled = 50, times = 1000, limit = 2, limit_exon = 5, nb = false)
    col = :Sample_Rep
    ...
    new_df[!,"TSS_BS"] =  bs
    new_df[!,"moment_BS"] =  (new_df[!,"bootstrap_std"].^2)./ new_df[!,"bootstrap_mean"]
    new_df[!,"TSS_BF"] =  bf
    new_df[!,"moment_BF"] = new_df[!,"bootstrap_mean"] ./ ( new_df[!,"bootstrap_std"].-1)
