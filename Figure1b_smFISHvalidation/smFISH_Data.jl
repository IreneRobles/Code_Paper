...
function smFISHdata(gene)
    ...
    if in(GENE, ["ACTB", "IL12B", "STAT1"])
        return CSV.read(smFISH_Data.folder*"/smFISH_validation/actb_stat1_il12b.csv", nullable=false, DataFrames.DataFrame)
    elseif in(GENE, ["ATF3", "TNF", "CD40"])
        return CSV.read(smFISH_Data.folder*"/smFISH_validation/cd40_atf3_tnf.csv", nullable=false, DataFrames.DataFrame)
    end
end
